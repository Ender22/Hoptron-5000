...
- [x] Playtest round 2 fixes (overnight 2026-06-11/12): **level "4x shorter" bug** — kills of carried-over enemies counted toward the next segment's quota and cascaded with the 50% cuts; quota now counts same-segment spawns only (`Enemy.waveSegment`), and the spawn population gate also counts current-segment enemies only (else leftovers softlock a segment). **Boss fidelity audit**: 12-agent workflow compared every boss vs the original Boss_*.as → 40 high + 64 medium findings (saved: `notes/boss-audit-findings.json`), ALL applied by a fix agent — watermelon seed axis reversed (p0−p1), return-side from player.x, original action loops, Eggplant inclusive-rand (uppercut was unreachable), Pumpkin/MagicMan/Burrito side-blasts are now true SUPER_BLAST beams (`ProjectileOpts.beam`), Hamburger fly-slam tween + heart-hit escape via `AttachedMinion.onHurtHook`, Combo 14-piece break at original −1000/−2000 HP steps, Burrito ham_suck/pk_sideblast/dur_flycrush forms added, MagicMan fartAndLaugh + teleportSmashForward ported. **Sundae/Burrito stun is now the REAL mechanic**: sword reflects projectiles (`EnemyProjectiles.slashAt`, reflected shots hurt enemies via `enemyHitTest`; invincible boss → blocked-hit → stun counter). **Title flow**: splash (original MainTitleBG art, PRESS ENTER) → setup screen (level+loadout); HUD/world hidden at title (was leaking "Score 0" behind the logo). **Progression surfaced**: death → run tally (kills/score/+AP) with retry/title/S-shop controls; AP shop opens from the tally too (Devin: "progression feels missing" — it was hidden behind the S hint).
- [ ] **MORNING VERIFY (browser was frozen overnight — rAF/CDP throttled)**: title splash→setup→start; die → tally → S opens shop → buy → retry; watermelon seeds fire OUT of the gun toward the player; level 1 pacing ≈50% shorter (not 4x); slash a Sundae cherry back into it (level 5 boss) → stun; Pumpkin sideblast beam; Combo 14-piece break; MagicMan fart + teleport smash; cake candles ×6 w/ flames.
- [ ] Tuning pass round 3 with Devin: combo tier thresholds, spell cooldowns, trail color verdict, dash trail VFX, hit sparks.
